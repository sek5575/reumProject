 ...
 */
@WebServlet("/input_tag_jquery")  //보여지는 주소이면서 jsp에서 보내는 값들을 받는 주소이다.
...
//		forward은  데이터베이스의 insert 라인을 넣으면 안된다. 새로고침하면 계속 데이터가 갱신되서 들어가게 된다.
		...
		response.sendRedirect("/input_tag_result3.jsp?uid="+uid+"&upw="+upw+"&gen="+gen+
				      "&tel="+tel+"&content="+content+"&ufile="+ufile+hprm);  // 저 주소로 이동열어라 (페이지 전환을 한다)
	}
